...
export async function POST(req) {
  ...
  try {
    ...
    payload.user_type =
      payload.biz_type === "corporate_business" ? "법인" : "개인";
    ...
    const dbPayload = {
      ...
      user_type: nullIfEmpty(payload.user_type),
      ...
      ssn: isIndividual(biz_type)
        ...
        : null,
      identification_type:
        isIndividual(biz_type) && is_minor === "N" && is_foreigner === "N"
      ...
      corp_name:
        biz_type === "corporate_business"
          ? nullIfEmpty(payload.corp_name)
          : null,
      corp_reg_no:
        biz_type === "corporate_business"
          ? nullIfEmpty(payload.corp_reg_no)
          : null,

      guardian_name:
        is_minor === "Y" ? nullIfEmpty(payload.guardian_name) : null,
      guardian_tel: is_minor === "Y" ? nullIfEmpty(payload.guardian_tel) : null,
    };
    ...
    if (existingFilesS3KeysToDelete.length > 0) {
      try {
        await Promise.all(
          existingFilesS3KeysToDelete.map((key) => deleteFileFromS3(key))
        );
      } catch {}
    }
  }

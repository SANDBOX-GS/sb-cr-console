    ...
    try {
        ...
        for (const payee of targetPayees) {
            ...
            try {
                ...
                // [STEP 3-2] 이메일 발송
                // -------------------------------------------------------------
                const emailRes = await sendNHNEmail(
                    payee.member_email,
                    displayName,
                    sendParams
                );
                ...
                if (emailResult.success)
                    console.log(`📧 Email Sent: ${payee.member_email}`);
                else
                    console.error(
                        `❌ Email Fail: ${payee.member_email} (${emailResult.message})`
                    );
                ...
                if (payee.member_tel && payee.member_tel.length > 9) {
                    const kakaoRes = await sendNHNKakao(
                        payee.member_tel,
                        sendParams
                    );
                    kakaoResult = {
                    ...
                    if (kakaoResult.success)
                        console.log(`💬 Kakao Sent: ${payee.member_tel}`);
                    else
                        console.error(
                            `❌ Kakao Fail: ${payee.member_tel} (${kakaoResult.message})`
                        );
                } else {
                ...
                console.error(
                    `❌ Process Error for payee idx ${payee.payee_idx}:`,
                    innerError.message
                );
                results.push({
                    idx: payee.payee_idx,
